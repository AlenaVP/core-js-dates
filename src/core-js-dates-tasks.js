...
function getTime(date) {
  ...
  return `${String(myDate.getHours()).padStart(2, '0')}:${String(myDate.getMinutes()).padStart(2, '0')}:${String(myDate.getSeconds()).padStart(2, '0')}`;
}
...
function getDayName(date) {
  const myDate = new Date(date);
  return new Intl.DateTimeFormat('en-US', { weekday: 'long' }).format(myDate);
}
...
function getNextFriday(date) {
  ...
  const weekday = myDay.getDay();
  return new Date(
    Date.parse(myDay) + ((weekday < 5 ? 5 : 12) - weekday) * MILLISEC_PER_DAY
  );
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  return (
    1 + Math.round((new Date(dateEnd) - new Date(dateStart)) / MILLISEC_PER_DAY)
  );
}
...
function isDateInPeriod(date, period) {
  const myDate = new Date(date);
  return myDate >= new Date(period.start) && new Date(period.end) >= myDate;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const countDaysOnPeriod = getCountDaysOnPeriod(startDate, endDate);
  const schedule = [];

  for (let i = 0; i <= countDaysOnPeriod; i += countWorkDays + countOffDays) {
    for (let workday = 0; workday < countWorkDays; workday += 1) {
      const currentDate = new Date(
        startDate.getTime() + (i + workday) * MILLISEC_PER_DAY
      );

      if (currentDate > endDate) break;
      const dateStr = currentDate.toLocaleDateString('en-GB');
      schedule.push(dateStr.replaceAll('/', '-'));
    }
  }
